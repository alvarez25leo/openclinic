...
import be.openclinic.statistics.DatacenterHospitalStats;
import net.admin.*;

public class PDFDatacenterHospitalReportGenerator extends PDFOfficialBasic {
    Date begin;
    Date end;
    HashSet chapters;
    DatacenterHospitalStats hospitalStats = null;
    private final int pageWidth = 100;
    private String type;
    PdfWriter docWriter=null;
    Hashtable allPrestations=Prestation.getAll();
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFDatacenterHospitalReportGenerator(User user, String sProject, String sPrintLanguage, Date begin, Date end, HashSet chapters){
        this.user = user;
    ...
    //--- CONSTRUCTOR -----------------------------------------------------------------------------
    public PDFDatacenterHospitalReportGenerator(User user, String sProject, String sPrintLanguage, Date begin, Date end, HashSet chapters,String reportid){
        this.user = user;
        this.sProject = sProject;
        this.sPrintLanguage = sPrintLanguage;
        this.begin=begin;
    ...
    public ByteArrayOutputStream generatePDFDocumentBytes(final HttpServletRequest req) throws Exception {
		...
		try{
            ...
            doc.open();
            MedwanQuery.getInstance().setProgressValue(reportid,5);
            hospitalStats = new DatacenterHospitalStats(begin,end);
            hospitalStats.loadEncounters("icd10");
            MedwanQuery.getInstance().setProgressValue(reportid,15);
            printReportHeader();
            MedwanQuery.getInstance().setProgressValue(reportid,25);
            if(chapters.contains("1") || chapters.contains("2")){
            	printBaseActivityInformation();
    	    	doc.newPage();
            }
            MedwanQuery.getInstance().setProgressValue(reportid,35);
            if(chapters.contains("3")){
		    	printPathologyProfile();
		    	doc.newPage();
            }
            MedwanQuery.getInstance().setProgressValue(reportid,50);
            if(chapters.contains("4")){
			    printComorbidityComortality();
		    	doc.newPage();
            }
            MedwanQuery.getInstance().setProgressValue(reportid,65);
            if(chapters.contains("5")){
		    	printHealthEconometrics();
		    	doc.newPage();
            }
            MedwanQuery.getInstance().setProgressValue(reportid,80);
            if(chapters.contains("6")){
		    	printInsuranceProfile();
		    	doc.newPage();
            }
            MedwanQuery.getInstance().setProgressValue(reportid,95);
}
		catch(Exception e){
			baosPDF.reset();
			e.printStackTrace();
        ...
        MedwanQuery.getInstance().setProgressValue(reportid,100);
    ...
    private void printHealthEconometrics(){
    	try{
        	...
        	for(int n=0;n<kv.length;n++){
	    		if(kv[n]!=null){
		    		prestation = (Prestation)allPrestations.get(kv[n].getKey());
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 68);
	    	table.addCell(createBorderlessCell(" ",100));
	    	//Graphs
	    	...
	    	double t=hospitalStats.getTotalCost("admission");
	    	for(int n=0;n<kv.length;n++){
	    		if(kv[n].getKey().trim().equalsIgnoreCase("") || kv[n].getKey().equalsIgnoreCase("?")){
	    			redistributionfactor=t/(t-Double.parseDouble(kv[n].getValue()));
	    			break;
	    		}
	    	}
	    	String costcenter;
	    	for(int n=0;n<kv.length;n++){
	    		costcenter = ScreenHelper.getTranNoLink("costcenter", kv[n].getKey(), sPrintLanguage); 
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 70);
	    	table.addCell(createBorderlessCell(" ",100));
	    	...
	    	table.addCell(cell);
	    	redistributionfactor = 1;
	    	t=hospitalStats.getTotalCost("visit");
	    	for(int n=0;n<kv.length;n++){
	    		if(kv[n].getKey().trim().equalsIgnoreCase("") || kv[n].getKey().equalsIgnoreCase("?")){
	    			redistributionfactor=t/(t-Double.parseDouble(kv[n].getValue()));
	    			break;
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 73);
        	
        	Service service;
	    	for(int n=0;n<kv.length;n++){
	    		service = (Service)allServices.get(kv[n].getKey());
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 75);
	    	...
	    	for(int n=0;n<incomedepartments.length;n++){
	    		service = (Service)allServices.get(incomedepartments[n]);
	    		label=incomedepartments[n]+(service!=null?": "+service.getLabel(sPrintLanguage):"?");
		    	//Graphs
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 78);
	    	doc.add(table);
	    	doc.newPage();
	    	table =new PdfPTable(100);
	    	table.setWidthPercentage(100);
	        ...
	        cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
	    	table.addCell(cell);
	    	cell = new PdfPCell(new Paragraph(ScreenHelper.getTranNoLink("hospital.statistics", "income.and.income.per.case", sPrintLanguage),FontFactory.getFont(FontFactory.HELVETICA,6,Font.NORMAL)));
	        cell.setPaddingTop(5);
	        cell.setColspan(25);
	        cell.setBorder(PdfPCell.BOX);
	        cell.setBorderColor(innerBorderColor);
	    	...
	    	table.addCell(cell);
	    	redistributionfactor = 1;
	    	t=hospitalStats.getTotalCost("admission");
	    	for(int n=0;n<kv.length;n++){
	    		if(kv[n].getKey().trim().equalsIgnoreCase("") || kv[n].getKey().equalsIgnoreCase("?")){
	    			redistributionfactor=t/(t-Double.parseDouble(kv[n].getValue()));
	    			break;
	    	...
	    	for(int n=0;n<kv.length;n++){
		        ...
		        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
		    	table.addCell(cell);
		    	numberOfCases=hospitalStats.getAdmissionWithDeliveriesDiagnosisCount(kv[n].getKey());
	    	...
	    	for(int n=0;n<kv.length;n++){
		        ...
		        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
		    	table.addCell(cell);
		    	numberOfCases=hospitalStats.getVisitWithDeliveriesDiagnosisCount(kv[n].getKey());
		    	cell = new PdfPCell(new Paragraph(new DecimalFormat(MedwanQuery.getInstance().getConfigString("reportPriceFormat","#,###.00")).format(Double.parseDouble(kv[n].getValue()))+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")+(kv[n].getKey().equalsIgnoreCase("?")?"":" ("+new DecimalFormat(MedwanQuery.getInstance().getConfigString("reportPriceFormat","#,###.00")).format(Double.parseDouble(kv[n].getValue())*redistributionfactor)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")+")"+
    			"\n"+new DecimalFormat(MedwanQuery.getInstance().getConfigString("reportPriceFormat","#,###.00")).format(Double.parseDouble(kv[n].getValue())/numberOfCases)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")+(kv[n].getKey().equalsIgnoreCase("?")?"":" ("+new DecimalFormat(MedwanQuery.getInstance().getConfigString("reportPriceFormat","#,###.00")).format(Double.parseDouble(kv[n].getValue())*redistributionfactor/numberOfCases)+" "+MedwanQuery.getInstance().getConfigString("currency","EUR")+")")),FontFactory.getFont(FontFactory.HELVETICA,6,Font.NORMAL)));
		        cell.setPaddingTop(5);
		        cell.setColspan(25);
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 79);
	    	table.addCell(createBorderlessCell(" ",100));	    	
	    	//Graphs
	        ...
	        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
	    	table.addCell(cell);
	    	cell=createLineChartCell((double[])resizeArray(hospitalStats.getWI(),hospitalStats.getWI().length-1),
    ...
    private void printComorbidityComortality(){
       	try{
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 53);
	    	if(counter%2!=0){
		    	table.addCell(createBorderlessCell(" ",50));
	    	}
	    	doc.add(table);
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 56);
	    	if(counter%2!=0){
		    	table.addCell(createBorderlessCell(" ",50));
	    	}
	    	doc.add(table);
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 59);
	    	if(counter%2!=0){
		    	table.addCell(createBorderlessCell(" ",50));
	    	}
	    	doc.add(table);
	    	...
	    	MedwanQuery.getInstance().setProgressValue(reportid, 62);
	    	if(counter%2!=0){
		    	table.addCell(createBorderlessCell(" ",50));
	    	}
	    	doc.add(table);
    ...
    private void printBaseActivityInformation(){
    	try{
	    	...
	    	if(chapters.contains("2")){
		    	...
		    	table.addCell(createBorderlessCell(" ",100));
		    	//Graphs
		    	cell=createLineChartCell((int[])resizeArray(hospitalStats.getWA(),hospitalStats.getWA().length-1),
		    			...
		    			true); //regression
		    	table.addCell(cell);
		    	cell=createLineChartCell((int[])resizeArray(hospitalStats.getWV(),hospitalStats.getWV().length-1),
		    	...
		    	cell=createLineChartCell(hospitalStats.getDAP(),
		    			...
		    			true); //regression
		    	table.addCell(cell);
		    	cell=createLineChartCell((int[])resizeArray(hospitalStats.getWD(),hospitalStats.getWD().length-1),
    ...
    private PdfPCell createKPGSBarChartCell(KeyValue[] values,String title,String xlabel,String ylabel) throws IOException, BadElementException{
        ...
        plot.setBackgroundPaint(new java.awt.Color(BaseColor.WHITE.getRGB()));
        plot.setRangeGridlinePaint(new java.awt.Color(BaseColor.GRAY.getRGB()));
        plot.setDomainGridlinePaint(new java.awt.Color(BaseColor.GRAY.getRGB()));
    ...
    private PdfPCell createPrestationMoneyFamilyPieChartCell(KeyValue[] values,String title) throws IOException, BadElementException{
        ...
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        ChartUtilities.writeChartAsJPEG(os,chart,MedwanQuery.getInstance().getConfigInt("stats.piechartwidth",640),MedwanQuery.getInstance().getConfigInt("stats.piechartheight",480));
        cell = new PdfPCell();
        cell.setColspan(50);
        cell.setImage(Image.getInstance(os.toByteArray()));
        cell.setBorder(PdfPCell.NO_BORDER);
	...
	private class PrestationLegendGenerator implements PieSectionLabelGenerator {
    	public String generateSectionLabel(final PieDataset dataset, final Comparable key) {
    		...
    		if (dataset != null) {
    			temp = key.toString();
    			Prestation prestation = (Prestation)allPrestations.get(temp);
	...
	private class ServiceLegendGenerator implements PieSectionLabelGenerator {
    	public String generateSectionLabel(final PieDataset dataset, final Comparable key) {
    		...
    		if (dataset != null) {
    			temp = key.toString();
    			Service service = (Service)allServices.get(temp);
