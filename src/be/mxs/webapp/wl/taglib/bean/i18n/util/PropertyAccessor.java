        ...
        if(key.trim().length()>0){
            if(getFormatType().equalsIgnoreCase("date")){
                try{
                    Date date = null;
                    try{
                        date = ScreenHelper.stdDateFormat.parse(key);
                    }
                        ...
                        if(_method.getReturnType().isAssignableFrom(java.lang.String.class)){
                            ...
                            if(objectValue==null){
                            	...
                            	if(formatType.equalsIgnoreCase("date")){
                            		java.util.Date parsedDate = ScreenHelper.parseDate((String)objectValue);
                            		objectValue = ScreenHelper.stdDateFormat.format(parsedDate);
                            	}
