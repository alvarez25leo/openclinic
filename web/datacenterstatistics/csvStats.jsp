<%@ page import="be.openclinic.statistics.CsvStats" %><%@include file="/includes/validateUser.jsp"%><%@ page import="be.mxs.common.util.db.MedwanQuery" %><%@ page import="java.text.SimpleDateFormat" %><%@ page import="java.util.Date" %><%
    ...
    else if("service.income.list".equalsIgnoreCase(request.getParameter("query"))){
        query="select "+ MedwanQuery.getInstance().convert("int","sum(oc_amount)")+" as total,"+ MedwanQuery.getInstance().convert("int","sum(oc_patientamount)")+" as patientincome,"+ MedwanQuery.getInstance().convert("int","sum(oc_insuraramount+oc_extrainsuraramount)")+" as insurarincome,oc_serviceuid as serviceuid,oc_prestationcode as prestation" +
		        " from updatestats4" +
		        " where oc_serviceuid is not null and oc_serviceuid<>'' and oc_enddate>"+MedwanQuery.getInstance().convertStringToDate("'<begin>'")+
		       	" and oc_begindate<"+MedwanQuery.getInstance().convertStringToDate("'<end>'")+
    ...
    CsvStats csvStats = new CsvStats(request.getParameter("begin"), request.getParameter("end"), "admin".equalsIgnoreCase(request.getParameter("db"))?lad_conn:"stats".equalsIgnoreCase(request.getParameter("db"))?stats_conn:loc_conn, query);
