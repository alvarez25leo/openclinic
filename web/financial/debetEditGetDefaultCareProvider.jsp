...
<%
	...
	if(performeruid.length()==0 && encounteruid.length()>0){
		...
		if(encounter != null){
			...
			if(service!=null && service.performeruid!=null && performeruid.length()>0){
			...
			while(performeruid.length()==0 && service!=null && service.parentcode!=null && service.parentcode.length()>0){
				...
				if(service!=null && service.performeruid!=null && service.performeruid.length()>0){
					performeruid=service.performeruid;
				}
			}
		}
	}
	if(performeruid!=null && performeruid.length()==0){
