...
<table width="100%">
    <tr>
        <td class="menuItem" colspan="2">
            <input type="submit" class="button" onmouseup="window.close();" value="<%= getTranNoLink("web","find",sWebLanguage)%>"/>
        </td>
    </tr>
...
<%
    ...
    while(iter.hasNext()){
        ...
        out.print("<tr>");
         out.print("<td class='menuItem'><input type='checkbox' name='cb"+counter+"' value='"+hLabHistoryListTranData.get("serverId")+"."+hLabHistoryListTranData.get("transactionId")+"' "+(counter<5?"checked":"")+"/></td>"+
                   "<td class='menuItem'>"+dateFormat.format(hLabHistoryListTranData.get("updateTime"))+"</td>");
        out.print("</tr>");
