...
<%
	if(session==null || session.isNew() || activeUser==null){
	    out.print("<script>window.location.href='"+sCONTEXTPATH+"/mpiLogin.jsp;</script>");
	    out.flush();
	}
...
<header class="nav-wrapper -sticky">
  <nav class="nav">
    ...
    <ul class="nav-list" role="navigation">
      <div class="list -left">
      	<%if(SH.c(activeUser.getParameter("patientid")).length()==0){ %>
        <li class="item">
        ...
        <%} %>
        <li class="item" id='menu_pacs' style='display: <%=SH.c(activeUser.getParameter("patientid")).length()==0?"none":""%>'>
        ...
        <li class="item"  style='display: <%=SH.c(activeUser.getParameter("onetime")).length()>0?"none":""%>'>
